...
public class Coach extends Thread {
     ...
     */
    public Coach(int team, boolean method, RefereeSite refereeSite, Playground playground, ContestantsBench contestantsBench) {
    ...
    public void run() {
        int orders;
        Pair<Integer, Integer>[] contestants = contestantsBench.reviewNotes(); // Review notes to help with assembling the team
        int[] roster = selectPlayers(contestants); // Select players for the game
        while (true) {
            orders = contestantsBench.wait_for_referee_command(); // Wait for referee's command
            if (orders == 0) return; // Terminate if no further orders
            contestantsBench.callContestants(roster); // Call selected contestants
            playground.assemble_team(); // Assemble team on the playground
            refereeSite.informReferee(); // Inform referee that team is ready
            playground.watch_trial(); // Watch the trial on the playground
            contestants = contestantsBench.reviewNotes(); // Review notes for next game
