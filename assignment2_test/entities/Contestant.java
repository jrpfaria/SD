...
public class Contestant extends Thread {
    ...
    public void run() {
        int orders;
        while (true) {
            orders = contestantsBench.seat_at_the_bench(); // Wait for instructions while seated at the bench
            switch (orders) {
            ...
            playground.followCoachAdvice(); // Follow coach's advice
            playground.stand_in_position(); // Stand in position on the playground
            playground.getReady(); // Get ready for the trial
            pullTheRope();
            playground.amDone();
            contestantsBench.seatDown(); // Seat down after the trial
        }
    }
