...
public class ServerGameOfRopeGeneralRepos {
     ...
     * args[2] - port nunber where the registering service is listening to service
     * requests
     * 
    ...
    public static void main(String[] args) {
        ...
        try {
            while (!end)
                synchronized (Class.forName("serverSide.main.ServerGameOfRopeGeneralRepos")) {
                    try {
                        (Class.forName("serverSide.main.ServerGameOfRopeGeneralRepos")).wait();
                    } catch (InterruptedException e) {
                        GenericIO.writelnString("General Repository main thread was interrupted!");
                    }
                }
        } catch (ClassNotFoundException e) {
            GenericIO.writelnString("The data type ServerGameOfRopeGeneralRepos was not found (blocking)!");
            e.printStackTrace();
            System.exit(1);
        }
    ...
    public static void shutdown() {
        ...
        try {
            synchronized (Class.forName("serverSide.main.ServerGameOfRopeGeneralRepos")) {
                (Class.forName("serverSide.main.ServerGameOfRopeGeneralRepos")).notify();
            }
        } catch (ClassNotFoundException e) {
            GenericIO.writelnString("The data type ServerGameOfRopeGeneralRepos was not found (waking up)!");
            e.printStackTrace();
            System.exit(1);
        }
    }
